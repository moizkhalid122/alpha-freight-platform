  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled ? (isDark ? "bg-white/80 backdrop-blur-md py-4 border-b border-slate-100" : "bg-black/20 backdrop-blur-md py-4") : "bg-transparent py-8"
      }`}
    >
      <div className="max-w-[1800px] mx-auto px-6 lg:px-12 flex justify-between items-center">
        ...
        <Link href="/" className="flex items-center group">
          ...
          <span className={`text-xl font-bold tracking-tighter ${isDark ? "text-slate-900" : "text-white"}`}>
            ALPHA FREIGHT
          </span>
        </Link>
                  ...
                  >
                    <div className="w-full max-w-[1200px] bg-white/95 backdrop-blur-2xl border border-white/20 rounded-3xl overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,0.4)] pointer-events-auto p-12">
                      ...
                      <div className="mt-12 pt-8 border-t border-black/5 flex items-center justify-between">
                        ...
                        <Link href="/demo" className="text-[11px] font-bold text-blue-600 uppercase tracking-widest hover:text-blue-700 transition-colors flex items-center gap-2">
                          Request a live demo
        ...
        {/* Mobile Menu Toggle */}
        <button
          className={`lg:hidden p-2 ${isDark ? "text-slate-900" : "text-white"}`}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className={`fixed inset-0 z-[60] flex flex-col p-10 pt-32 overflow-y-auto ${
              isDark ? "bg-white text-slate-900" : "bg-black text-white"
            }`}
          >
            <button 
              onClick={() => setMobileMenuOpen(false)}
              className={`absolute top-10 right-10 text-3xl p-2 ${isDark ? "text-slate-900" : "text-white"}`}
            >
              &times;
            </button>
            ...
            <div className="flex flex-col space-y-6 w-full max-w-md mx-auto">
                ...
                <div key={link.name} className="flex flex-col space-y-4">
                  <div className="flex items-center justify-between">
                    <Link
                      ...
                      onClick={() => !(link.dropdown || link.megaMenu) && setMobileMenuOpen(false)}
                    >
                      {link.name}
                    </Link>
                    {(link.dropdown || (link.megaMenu && link.megaMenu.length > 0)) && (
                      <button 
                        onClick={() => setHoveredLink(hoveredLink === link.name ? null : link.name)}
                        className="p-2 text-white/50"
                      >
                        <svg className={`w-6 h-6 transition-transform ${hoveredLink === link.name ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                        </svg>
                      </button>
                    )}
                  ...
                  {(link.dropdown || (link.megaMenu && link.megaMenu.length > 0)) && hoveredLink === link.name && (
                    <motion.div 
                      initial={{ height: 0, opacity: 0 }}
                      animate={{ height: "auto", opacity: 1 }}
                      className="flex flex-col space-y-4 pl-6 border-l border-white/10"
                    >
                      {link.dropdown?.map((item) => (
                        <Link
                          key={item.name}
                      ...
                      {link.megaMenu?.map((column) => (
                        <div key={column.category} className="space-y-4">
                          <div className="text-[10px] font-bold text-[#BFFF07] uppercase tracking-widest">{column.category}</div>
              ...
              <div className="pt-10 flex flex-col gap-4">
                <Link
                  href="/auth/select"
                  className="w-full py-5 rounded-2xl bg-white/10 text-white text-center font-bold uppercase tracking-widest border border-white/10"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Login
                </Link>
                <Link
                  href="/contact"
                  className="w-full py-5 rounded-2xl bg-[#BFFF07] text-black text-center font-bold uppercase tracking-widest"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Contact us
                </Link>
