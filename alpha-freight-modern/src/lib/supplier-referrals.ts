...
export const REFERRAL_MILESTONE_LOADS = 10;
export const REFERRAL_BASE_REWARD = 100;
...
export function buildReferralCode(userId: string) {
  return `AF-SUP-${userId.slice(0, 8).toUpperCase()}`;
}

export function buildReferralSignupLink(referralCode: string) {
...
export async function ensureSupplierReferralCode(supplierId: string) {
  const code = buildReferralCode(supplierId);
...
async function findReferrerByCode(referralCode: string): Promise<ProfileRow | null> {
  ...
  if (!storedError && byStoredCode?.role === "supplier") {
  ...
  if (suppliersError || !suppliers?.length) return null;

  const matched = (suppliers as ProfileRow[]).find(
    (profile) => buildReferralCode(profile.id) === normalized
  );

  return matched || null;
}

export async function validateSupplierReferralCode(referralCode: string) {
  const normalized = referralCode.trim().toUpperCase();
...
export async function recordSupplierReferralFromSignup(params: {
  ...
  const { error } = await supabase.from("supplier_referrals").insert([
    {
      referrer_id: referrer.id,
      referred_user_id: params.referredUserId,
      referral_code: referralCode,
      status: "pending",
      reward_amount: REFERRAL_BASE_REWARD,
      milestone_loads: REFERRAL_MILESTONE_LOADS,
    },
  ]);

  if (error && !isMissingTableError(error.message)) {
    console.warn("[supplier-referrals] Unable to record referral:", error.message);
  }
}

export async function fetchSupplierReferralDashboard(
  supplierId: string
): Promise<SupplierReferralDashboard> {
  ...
  for (const record of records) {
    ...
    const milestoneLoads = record.milestone_loads || REFERRAL_MILESTONE_LOADS;
    const rewardAmount = Number(record.reward_amount || REFERRAL_BASE_REWARD);
