...
import AirLoginTransitionOverlay from "@/components/air/AirLoginTransitionOverlay";
import {
  AIR_PORTAL,
  airDashboardPath,
...
export default function AirLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        const userRole = profile?.role || role || "supplier";
        const onboardingRaw =
          typeof window !== "undefined"
            ? localStorage.getItem(airOnboardingStorageKey(data.user.id))
            : null;
        const onboarded = onboardingRaw && JSON.parse(onboardingRaw)?.completedAt;
        const dashboard = onboarded
          ? airDashboardPath(userRole)
          : `${AIR_PORTAL.onboarding}?role=${userRole}`;
  ...
  return (
    <>
      <div className="mb-6">
        <h2 className="air-font-display text-2xl font-medium tracking-tight text-slate-900">Sign in to Air Freight</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-300 focus:ring-2 focus:ring-sky-500/10"
          />
        </div>
        ...
        <div>
          <div className="mb-1.5 flex items-center justify-between">
            ...
            <Link href="/auth/forgot-password" className="text-[10px] font-semibold uppercase tracking-wider text-sky-600">
              Forgot?
            </Link>
          </div>
          <div className="relative">
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 pr-11 text-sm text-slate-900 outline-none transition focus:border-sky-300 focus:ring-2 focus:ring-sky-500/10"
            />
            <button
              type="button"
      ...
      <p className="mt-5 text-center text-sm text-slate-500">
        ...
        <Link href="/auth/air/signup" className="font-bold text-slate-900 underline underline-offset-4">
          Create air account
        </Link>
      </p>

      <AirLoginTransitionOverlay
