...
export default function SupplierProfilePage() {
  ...
  const [supplier, setSupplier] = useState<PublicSupplierListing | null>(null);
  const [related, setRelated] = useState<PublicSupplierListing[]>([]);
  ...
  useEffect(() => {
    async function loadSupplier() {
      ...
      try {
        ...
        const payload = (await response.json()) as { suppliers?: PublicSupplierListing[] };
        const suppliers = payload.suppliers ?? [];
        const found = suppliers.find((item) => item.id === id) ?? null;

        setSupplier(found);
        setRelated(suppliers.filter((item) => item.id !== id).slice(0, 3));
  ...
  const toggleShortlist = () => {
    ...
    const next = isShortlisted
      ? shortlist.filter((item: string) => item !== supplier.id)
      : [...shortlist, supplier.id];

    localStorage.setItem("alpha_supplier_shortlist", JSON.stringify(next));
    window.dispatchEvent(new Event("alpha_shortlist_updated"));
    setIsShortlisted(!isShortlisted);
  };

  const contactHref = useMemo(() => {
    if (!supplier) return "/contact";
    const subject = encodeURIComponent(`Supplier enquiry: ${supplier.name}`);
    const body = encodeURIComponent(
      `Hello Alpha Freight team,\n\nI would like to connect with ${supplier.name} regarding freight requirements.\n\nThanks,`,
    );
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD]">
        ...
        <div className="mx-auto max-w-3xl px-6 pt-40 pb-24 text-center">
          <h1 className="text-3xl font-bold text-slate-900">Supplier not found</h1>
          <p className="mt-4 text-slate-500">
            This profile is not available or the supplier is not yet verified on Alpha Freight.
          </p>
          <Link
            href="/suppliers"
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="p-8 md:p-10 space-y-6">
              <div className="flex items-center gap-2 text-amber-500">
                ...
                <span className="text-lg font-bold text-slate-900">{supplier.rating.toFixed(1)}</span>
                <span className="text-sm text-slate-400">({supplier.reviews} completed loads)</span>
              </div>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <a
                  href={contactHref}
                  className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-5 py-3 text-[11px] font-bold uppercase tracking-[0.18em] text-white hover:bg-amber-500 transition-colors"
                >
        ...
        {related.length > 0 ? (
          <section className="mt-16">
            <div className="mb-8 flex items-end justify-between gap-4">
              <h2 className="text-2xl font-bold text-slate-900">More verified suppliers</h2>
              <Link href="/suppliers" className="text-sm font-semibold text-slate-500 hover:text-slate-900">
                View all
              </Link>
            </div>
