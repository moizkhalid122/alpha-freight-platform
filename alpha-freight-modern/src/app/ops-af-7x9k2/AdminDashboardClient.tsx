...
async function fetchOverviewFromSupabase() {
  ...
  return buildDashboardFromOverview({
    profiles: (profilesResult.error ? [] : profilesResult.data) as ProfileRecord[],
    loads: (loadsResult.error ? [] : loadsResult.data) as LoadRecord[],
    bids: (bidsResult.error ? [] : bidsResult.data) as BidRecord[],
  });
}
...
async function fetchOverview() {
  try {
    ...
    return buildDashboardFromOverview(overview);
  } catch (apiError) {
    console.warn("[admin dashboard] API overview failed, using direct Supabase:", apiError);
    return fetchOverviewFromSupabase();
  }
}

function buildDashboardFromOverview(overview: {
  profiles?: ProfileRecord[];
  loads?: LoadRecord[];
  bids?: BidRecord[];
}) {
  const profiles = overview.profiles ?? [];
  const loads = overview.loads ?? [];
      ...
      ? [
          {
            ...
            href: "/ops-af-7x9k2/loads/available",
            tone: "slate" as const,
          },
        ]
      : []),
...
export default function AdminDashboardClient({
  ...
  initialOverviewRaw?: AdminOverviewRaw | null;
}) {
  ...
  const seededDashboard = useMemo(
    () => (initialOverviewRaw ? buildDashboardFromOverview(initialOverviewRaw) : undefined),
    [initialOverviewRaw]
  );
  ...
  const { data, isPending, isFetching, isError, refetch, dataUpdatedAt } = useQuery({
    ...
    initialData: seededDashboard,
    placeholderData: (previous) => previous ?? seededDashboard,
    staleTime: 5 * 60_000,
    gcTime: 15 * 60_000,
  ...
  return (
    <div ref={dashboardRef} className="admin-page-stack space-y-4">
      ...
      >
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Link href="/ops-af-7x9k2/users/activity-log">
