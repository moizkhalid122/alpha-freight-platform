...
const MOTION_THRESHOLD_MPS = 0.45; // ~1 mph
...
export async function publishLoadGpsUpdate(input: GpsPublishInput): Promise<LoadLiveTracking | null> {
  const {
    data: { user },
  ...
  const liveRow = {
    ...
    heading: input.heading ?? null,
    accuracy_m: input.accuracyM ?? null,
    traveled_meters: snapshot.traveledMeters,
    motion_seconds: snapshot.motionSeconds,
    avg_speed_mps: snapshot.avgSpeedMps,
    is_moving: snapshot.isMoving,
    tracking_active: true,
    started_at: snapshot.startedAt,
    updated_at: snapshot.now,
    last_recorded_at: snapshot.now,
  };

  const { error: historyError } = await supabase.from("load_location_updates").insert(historyRow);
  if (historyError) {
    if (/load_location_updates|schema cache|does not exist/i.test(historyError.message)) {
      return null;
    }
    throw historyError;
  }
  ...
  const writeQuery = existingRow
    ? supabase.from("load_live_tracking").update(liveRow).eq("load_id", input.loadId)
    : supabase.from("load_live_tracking").insert(liveRow);

  const { data, error: liveError } = await writeQuery.select("*").single();

  if (liveError) {
    if (/load_live_tracking|schema cache|does not exist/i.test(liveError.message)) {
      return null;
    }
    throw liveError;
  }

  return data as LoadLiveTracking;
}

export async function stopLoadLiveTracking(loadId: string) {
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  if (error && !/load_live_tracking|schema cache|does not exist/i.test(error.message)) {
    throw error;
  }
}
...
export async function fetchCarrierInTransitLoads() {
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  const { data, error } = await supabase
    ...
    .eq("carrier_id", user.id)
    .in("status", ["in-transit", "loading"]);

  if (error) throw error;
  return (data ?? []) as { id: string; supplier_id: string; status: string }[];
}
