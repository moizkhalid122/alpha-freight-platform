...
// Send OTP Email
async function sendOTPEmail(email, otp, customSubject = null, customHTML = null) {
  const emailHTML = customHTML || getOTPEmailHTML(otp);
  const emailSubject = customSubject || (otp ? 'Your OTP Code - Alpha Freight' : 'Alpha Freight Notification');
...
app.post('/api/send-notification', async (req, res) => {
  try {
    ...
    if (!email || !templateId) {
      return res.status(400).json({ success: false, error: 'Email and templateId are required' });
    }

    let subject = '';
    let html = '';

    // Map old type format to new template IDs for backward compatibility
    const typeToTemplateId = {
      'subscription_renewal_reminder': emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_RENEWAL_REMINDER,
      'subscription_expired': emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_EXPIRED,
      'subscription_activated': emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_ACTIVATED,
      'subscription_cancelled': emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_CANCELLED,
      'insurance_renewal_reminder': emailTemplates.TEMPLATE_IDS.INSURANCE_RENEWAL_REMINDER,
      'payment_failed': emailTemplates.TEMPLATE_IDS.PAYMENT_FAILED
    };

    // Support both old 'type' and new 'templateId'
    const finalTemplateId = req.body.type ? typeToTemplateId[req.body.type] : templateId;

    try {
      html = emailTemplates.getTemplateById(finalTemplateId, data || {});
      
      // Set appropriate subject based on template
      const subjects = {
        [emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_RENEWAL_REMINDER]: `Subscription Renewal Reminder - ${data?.daysUntilRenewal || 7} Days Left`,
        [emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_ACTIVATED]: 'Subscription Activated - Welcome to Verified Carriers!',
        [emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_EXPIRED]: 'Subscription Expired - Renew to Continue Benefits',
        [emailTemplates.TEMPLATE_IDS.SUBSCRIPTION_CANCELLED]: 'Subscription Cancelled - Alpha Freight',
        [emailTemplates.TEMPLATE_IDS.INSURANCE_RENEWAL_REMINDER]: `Insurance Renewal Reminder - ${data?.daysRemaining || 0} Days Remaining`,
        [emailTemplates.TEMPLATE_IDS.PAYMENT_FAILED]: 'Payment Failed - Action Required'
      };
      
      subject = subjects[finalTemplateId] || 'Alpha Freight Notification';
    } catch (error) {
      return res.status(400).json({ success: false, error: `Invalid template ID: ${error.message}` });
    }

    const emailResult = await sendOTPEmail(email, null, subject, html);

    if (emailResult.success) {
      res.json({ success: true, message: 'Notification sent successfully' });
    } else {
      res.status(500).json({ success: false, error: emailResult.error || 'Failed to send notification' });
    }
  } catch (error) {
...
// Check subscription renewals (scheduled task endpoint)
app.post('/api/check-subscription-renewals', async (req, res) => {
  try {
    // This would typically be called by a cron job
    // For now, we'll return a success message
    // In production, this would check Firebase for expiring subscriptions
    
    res.json({ 
      success: true, 
      message: 'Subscription renewal check completed',
      checked: 0, // Would be actual count in production
      remindersSent: 0
    });
  } catch (error) {
...
// Catch-all handler: serve index.html for any non-API routes (for SPA routing)
...
app.listen(PORT, () => {
  ...
  console.log(`  http://localhost:${PORT}/pages/supplier/paynow.html\n`);
});
